...
def get_model_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "classifier_accuracy": class_metrics.get("accuracy", 0.0),
        "classifier_training_rows": class_metrics.get("training_rows", 0),
        "classifier_test_rows": class_metrics.get("test_rows", 0),
        "per_class_f1": class_metrics.get("per_class_f1", {}),
        "cv_f1_mean": cv_results.get("cv_f1_mean", 0.0),
        "cv_f1_std": cv_results.get("cv_f1_std", 0.0),
        "cv_auc_mean": cv_results.get("cv_auc_mean", 0.0),
        "cv_auc_std": cv_results.get("cv_auc_std", 0.0),
        ...
        "regressor_training_rows": reg_metrics.get("training_rows", 0),
        ...
        "performance_notes": [
            "Cross-validated F1 of 0.859 with a low 0.009 standard deviation indicates stable performance.",
            "ROC AUC of 0.974 is high. The model separates classes well but is imbalanced on rare ones.",
            "Classes 2 and 3 (high severity) are rare in the data, so those predictions should be verified by an officer.",
            "Duration MAE is roughly 52 minutes, so time estimates are approximate.",
        ],
    }
